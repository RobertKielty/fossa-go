...
// Scope the issue-list export to a single release group, using its latest release (the issues shown on the release group&#39;s Issues page). Requires view access to the release group; otherwise rejected with a &#x60;403&#x60;. When set, &#x60;teamIds&#x60; is ignored — the export is scoped to the release group only, not an intersection of both filters. When combined with &#x60;includeDailyCounts&#x60;, the bundle&#39;s counts CSV is &#x60;release_counts.csv&#x60; rather than the daily &#x60;issue_counts.csv&#x60;. 
...
Download the global issues report bundle (a zip of per-category issue-list CSVs and/or a counts CSV) or submit it for background processing.

The counts CSV differs by scope: team/organization scope yields `issue_counts.csv`, a daily time series, while a release-group scope yields `release_counts.csv`, a release-indexed comparison series.
...
// Filter ignored issues by ignore reason (category \&quot;vulnerability\&quot; or \&quot;licensing\&quot;). For vulnerabilities, this value appears in the vulnerabilities.analysis.detail field in CycloneDX SBOM reports. 

...
> *os.File GetGlobalIssuesCSV(ctx).Email(email).TeamIds(teamIds).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.IssuesAPI.GetGlobalIssuesCSV(context.Background()).Email(email).TeamIds(teamIds).Execute()
	if err != nil {

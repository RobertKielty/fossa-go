...
> UpdateIssues200Response GetSnippetCount(ctx, locator).Path(path).Ids(ids).PackageIds(packageIds).Search(search).RejectionStatus(rejectionStatus).PackageLabels(packageLabels).VendoredMatch(vendoredMatch).Execute()

...
> GetProjectDependency200Response GetGlobalDependency(ctx, locator).IncludeResolutionNotes(includeResolutionNotes).IncludeLicenseText(includeLicenseText).IncludeCopyright(includeCopyright).IncludeMatches(includeMatches).IncludeDownloadUrl(includeDownloadUrl).Execute()
